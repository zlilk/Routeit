...
userProfile.controller('ProfileController', ['$rootScope', '$scope', '$http', '$compile', function($rootScope, $scope, $http, $compile){
    ...
    $scope.showMyRoutes = function(){
        ...
        for(var i = 0; i<myRoutesArr.length; i++){
            ...
            '<input type="date" ng-model = "date'+i+'" class = "date"> <button class = "dateBtn" ng-click="updateDate(' + myRoutesArr[i].trip_id + ',date' + i + ',' + myRoutesArr[i].days_num + ')"> עדכן תאריך </button></section>';
            allMyRoutes+=route;
        }
